...
public class Allocator{
	...
	Allocator(){
		...
		quotaPriority.put("PWDWo", 4);
		quotaPriority.put("SC", 5);
		quotaPriority.put("ST", 5);
		quotaPriority.put("OBC", 5);
		quotaPriority.put("STWo", 6);
		quotaPriority.put("OBC-A", 7);
		quotaPriority.put("OBC-B", 7);
		quotaPriority.put("BC", 7);
		quotaPriority.put("BCA", 7);
		quotaPriority.put("SCA", 7);
	...
	void readCourses(String filename, boolean header){
		...
		try{
			...
			while( (line = br.readLine() ) != null ){
				...
				int bc = Integer.parseInt( token[14].trim() );
				int bca = Integer.parseInt( token[15].trim() );
				int sca = Integer.parseInt( token[16].trim() );
				int urWom = Integer.parseInt( token[17].trim() );
				int stWom = Integer.parseInt( token[18].trim() );
				int pwdWom = Integer.parseInt( token[19].trim() );
				int bcbWom = Integer.parseInt( token[20].trim() );

				Course course = new Course( courseId, hospitalCode, state, specialityCode, total, ur, obc, sc, st, pwd, obca, obcb, bc, bca, sca, urWom, stWom, pwdWom, bcbWom);
